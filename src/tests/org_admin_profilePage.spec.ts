...
test.describe('Org. Admin Profile Edit Test Suite', () => {
  ...
  test( '@smoke @regression TC_PE_01 - Verify default tab selection on profile page',
    async ({ page }, testInfo) => {
  ...
  test( '@smoke @regression TC_PE_02 - Verify tab switching functionality',
    async ({ page }, testInfo) => {
      const homePage            = new HomePage(page, testInfo.title);
      const orgadminprofilePage = new OrgAdminProfilePage(page, testInfo.title);
      ...
      await expect(orgadminprofilePage.getOrganizationProfileTab()).toHaveAttribute('aria-selected', 'true');
  ...
  test( '@regression TC_PE_03 - Verify dropdown expand/collapse under License menu',
    async ({ page }, testInfo) => {
      const homePage            = new HomePage(page, testInfo.title);
      const orgadminprofilePage = new OrgAdminProfilePage(page, testInfo.title);
  ...
  test( '@regression TC_PE_04 - Verify popup open & close on license toggle',
    async ({ page }, testInfo) => {
      const homePage            = new HomePage(page, testInfo.title);
      const orgadminprofilePage = new OrgAdminProfilePage(page, testInfo.title);
      ...
      Logger.info(testInfo.title, `License toggle permission flow (${actionType}) popup cancel verified`);
      Logger.info(testInfo.title, `Popup cancel verified for ${actionType}`);
    },
  );

  test('@regression TC_PE_05 - Verify full license toggle flow (approve/remove)',
    async ({ page }, testInfo) => {
  ...
  test( '@smoke @regression TC_PE_06 - Verify edit icon click opens Update Admin Profile popup',
    async ({ page }, testInfo) => {
      const homePage            = new HomePage(page, testInfo.title);
      const orgadminprofilePage = new OrgAdminProfilePage(page, testInfo.title);
      ...
      await expect(orgadminprofilePage.getUpdateAdminProfileCancelIcon()).toBeVisible({ timeout: 5000 });
    },
  );

  test( '@regression TC_PE_07 - Verify form fields are pre-filled with existing data',
    async ({ page }, testInfo) => {
      const homePage            = new HomePage(page, testInfo.title);
      const orgadminprofilePage = new OrgAdminProfilePage(page, testInfo.title);
  ...
  test('@regression TC_PE_08 - Verify Update Admin Profile popup can be closed without submitting',
    async ({ page }, testInfo) => {
      const homePage            = new HomePage(page, testInfo.title);
      const orgadminprofilePage = new OrgAdminProfilePage(page, testInfo.title);
